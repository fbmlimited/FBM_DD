table 70007 FBM_ApprovalEntryM
{
    ...
    local procedure OnAfterGetRecordToApprove(ApprovalEntry: Record FBM_ApprovalEntryM; var RecRef: RecordRef; var Result: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterGetCustVendorDetails(var ApprovalEntry: Record FBM_ApprovalEntryM; var CustVendorNo: Code[20]; var CustVendorName: Text[100])
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeMarkAllWhereUserisApproverOrSender(var ApprovalEntry: Record FBM_ApprovalEntryM; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeShowRecord(var ApprovalEntry: Record FBM_ApprovalEntryM; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRecordDetails(var ApprovalEntry: Record FBM_ApprovalEntryM; var Details: Text; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRecordCaption(var ApprovalEntry: Record FBM_ApprovalEntryM; var Result: Text; var IsHandled: Boolean)
    begin
    end;
}
